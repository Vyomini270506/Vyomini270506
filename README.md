# Hi, I'm Vyomini Joshi 👋

🎓 BTech CSE @ NIT Surat
💻  DSA | Full Stack
---

## 👩‍💻 About Me

* 💡 Interests: AI/Ml, Web Development,Fintech
* 📍 Based in India
* 🎯 Solved 300+ problems on LeetCode
* 🏆 Winner – Google Winter of Code 2026

---

## 🛠️ Tech Stack

**Languages:**
C++, Python, C, JavaScript

**Frontend:**
HTML, CSS, React.js, Tailwind CSS

**Backend:**
Node.js, Express.js

**Database & Tools:**
MySQL, Firebase, Git, GitHub, VS Code

**Other:**
Arduino, IoT Systems

---

## 🚀 Projects

### ☕ Rabuste Café

* Full-stack MERN website with **story-driven UI**
* Built a **coffee recommendation system** using LLM APIs
* Integrated **Firebase authentication & real-time data**
* Generated **sales analytics dashboard for owners**

---

### 🛰️ CanSat (Mini Satellite)

* Built using ESP32, MPU6050, BMP180, LoRa
* Measures altitude, pressure, temperature
* Real-time **data transmission to ground station**
* Tracks acceleration and orientation

---

### 🧁 Bakery Management System

* Built with Python + MySQL
* Automated inventory, billing & sales tracking
* Simplified daily operations

---

## 🏆 Achievements

* 🥇 Google Winter of Code 2026 – Winner
* 🏁 Web Wonders 2025 – Finalist
* 🎓 INSPIRE-MANAK Scholar (DST Govt. of India)

---

## 💼 Positions of Responsibility

* Executive – E-Cell, NIT Surat
* Representative – Nexus, NIT Surat
* Volunteer – NSS

---

## 📫 Connect with Me

* 📧 Email: [vyominijoshi01@gmail.com](mailto:vyominijoshi01@gmail.com)
* 💼 LinkedIn: https://www.linkedin.com/in/vyomini-joshi-30aa63327/
* 💻 GitHub: https://github.com/Vyomini270506


---

⭐ *Always building, always learning.*
